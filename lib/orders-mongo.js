...
function normalizeOrderItems(items = []) {
  ...
  return items.map(item => {
    ...
    if (item.delivered) {
      ...
      // UI-only readiness for undelivered lines; never implies delivered.
      normalized.ready = Boolean(item.ready);
      normalized.hold = Boolean(item.hold);
    }
    return normalized;
  });
}
...
async function listOrders({ scope = 'all' } = {}) {
  ...
  if (scope === 'active') {
    // Everything except delivered + paid (past orders) and wish orders.
    query = {
      status: { $ne: 'wish' },
      $or: [
        { status: { $ne: 'delivered' } },
...
async function saveOrder(raw = {}, fileName) {
  ...
  if (payload.status === 'wish') {
    // No stock yet, so no bill to draft until the order leaves wish status.
